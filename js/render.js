...
//Import Data
import { dataMujeres, dataHombres, categorias } from "/js/data.js";
...
//Import Events
import { facturaContainer } from "/js/events.js";
/* ==============================
...
function detallesCompra() {
  ...
  const metodos = [
    "Assets/Img-pagos/visa.png",
    "Assets/Img-pagos/mastercard.png",
    "Assets/Img-pagos/paypal.png",
  ];
...
function imgZoom(imgElement, initialIndex = 0) {
  ...
  imgIzquierda.src = "Assets/left.png";
  imgIzquierda.alt = "Anterior";
  ...
  imgDerecha.src = "Assets/right.png";
  imgDerecha.alt = "Siguiente";
...
function contadorProducto(productoId, talla, cantidadInicial = 1) {
  ...
  // 🔥 Forzar valor después de que existan las opciones
...
function crearBtnfavorito(idProducto) {
  ...
  imgFav.src = "Assets/favorito1.png";
  imgFav.alt = "Agregar a Favoritos";
  ...
  imgRellenoFavorito.src = "Assets/favorito2.png";
  imgRellenoFavorito.alt = "Agregar a Favoritos";
...
/*Sincronizar Btn de Favorito 999*/
...
/*Sincronizar Btn de Carrito 999*/
...
function renderHistorialPedidos() {
  ...
  historialPedidos.forEach((pedido) => {
    ...
    p3.textContent = `Total: $${pedido.total}`;

    const p4 = document.createElement("p");
    p4.textContent = `Fecha: ${pedido.fecha}`;

    const p5 = document.createElement("p");
    p5.textContent = `Estado: ${pedido.estado.toUpperCase()}`;
    ...
    pedido.productos.forEach((item) => {
      ...
      img.descripcion = item.descripcion;
      img.loading = "lazy";
...
export function renderImgUsuario() {
  ...
  if (usuarioActivo) {
    /*
    img.src = "Assets/userlogInActivo.png";
    //img.alt = state.usuarioActivo.nombre;
    img.classList.add("userlogInActivo");
    logInLogo.append(img);*/ console.log("El Usuario se Logueo");
  }
...
function renderEstadoTabla() {
  ...
  estadosPedidosTabla.forEach((pedido) => {
    ...
    celdaTotal.textContent = `$${parseFloat(pedido.total || 0).toFixed(2)}`;
...
function renderPedidoDetalle(pedido) {
  ...
  info.innerHTML = `
        ...
        <div class="info-item">
            <strong>Total:</strong> $${parseFloat(pedido.total || 0).toFixed(2)}
        </div>
    `;
  ...
  pedido.productos.forEach((prod) => {
    ...
    productoDiv.innerHTML = `
            ...
            <div class="producto-info">
                ...
                <p><strong>Precio unit.:</strong> $${parseFloat(prod.precio).toFixed(2)}</p>
            </div>
        `;
...
/*Renderizar Factura */
export function renderFacturaPedido(pedido) {
  if (!pedido) {
    return;
  }
  //Para que encuentre el contenedor cuando se llame ya que es un template
  const facturasContainer = document.querySelector(".facturas-container");

  const template = document
    .getElementById("factura-template")
    .content.cloneNode(true);
  //Datos del Cliente
  /*
  const direccion = "Av. Siempre Viva 1212";
  const correo = pedido.correo;
  const telefonoCliente = pedido.telefono;
  */

  const IVA_PORCENTAJE = 0.09;
  const DESCUENTO = 9.36;

  let subtotal = 0;

  const tbody = template.querySelector("#factura-productos");
  tbody.innerHTML = "";

  // ===  Pedido  ===
  pedido.productos.forEach((item) => {
    const cantidad = item._cantidad;
    const precio = item.precio;

    const ivaProducto = precio * IVA_PORCENTAJE;
    const totalProducto = (precio + ivaProducto) * cantidad;

    subtotal += precio * cantidad;

    const tr = document.createElement("tr");
    tr.innerHTML = `
      <td>${item.alt || "Producto"}</td>
      <td>${cantidad}</td>
      <td>$${precio.toFixed(2)}</td>
      <td>$${ivaProducto.toFixed(2)}</td>
      <td>$0</td>
  ...
  template.querySelector("#cliente-nombre").textContent = pedido.usuario;

  const ivaTotal = subtotal * IVA_PORCENTAJE;
  const totalFinal = subtotal + ivaTotal - DESCUENTO;

  template.querySelector("#factura-fecha").textContent = pedido.fecha;
  template.querySelector("#factura-subtotal").textContent =
    "$" + subtotal.toFixed(2);
  template.querySelector("#factura-iva").textContent =
    "$" + ivaTotal.toFixed(2);
  template.querySelector("#factura-descuento").textContent =
    "$" + DESCUENTO.toFixed(2);
  template.querySelector("#factura-total-final").textContent =
    "$" + totalFinal.toFixed(2);
  template.querySelector("#factura-total").textContent =
    "$" + totalFinal.toFixed(2);
  // Datos del cliente
  /*
  template.querySelector("#cliente-direccion").textContent = direccion;
  template.querySelector("#cliente-correo").textContent = correo;
  template.querySelector("#cliente-telefono").textContent = telefonoCliente;*/

  facturasContainer.appendChild(template.firstElementChild);
}
renderFacturaPedido();
...
selectTipoTarjeta?.addEventListener("change", () => {
  ...
  if (metodoSeleccionado === "Visa") {
    ...
    imgTipoVisaMastercard.src = "Assets/Img-pagos/visa2.png";
  } else if (metodoSeleccionado === "Mastercard") {
    // Cambiamos la imagen al logo de Mastercard
    imgTipoVisaMastercard.src = "Assets/Img-pagos/mastercard2.png";
  }
});
...
export function renderPrincipalResize() {
  ...
  if (facturaContainer?.classList.contains("activado")) {
    facturaContainer.style.padding = width >= 344 ? "24px" : "0px";
  }
...
/* */
// Ejemplo de JS para agregar clase al body cuando se abre un menú
...
document.querySelectorAll(".btn-cerrar-categoria").forEach((btn) => {
  btn.addEventListener("click", () => {
...
export function renderDashboard() {
  ...
  pedidosActivos.forEach((pedido) => {
    ...
    const tdTotal = createCell(`$${parseFloat(pedido.total || 0).toFixed(2)}`);
...
function calcularEstadisticas() {
  ...
  // Función auxiliar para calcular ingresos por período
